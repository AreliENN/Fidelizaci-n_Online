...
<head>
    ...
    <style>
        body { height: 100vh; background: linear-gradient(to right, #f3f4f6, #e2e8f0); display: flex; align-items: center; justify-content: center; font-family: 'Segoe UI', sans-serif; }
        .login-form { background: #fff; padding: 3rem 3.5rem; border-radius: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.25); width: 100%; max-width: 500px; text-align: center; }
        .login-form h3 { font-size: 2rem; margin-bottom: 2rem; }
        .form-control { margin-bottom: 1.5rem; padding: 1rem; font-size: 1.1rem; border-radius: 10px; border: 1px solid #d1d5db; }
        .btn-login { width: 100%; padding: 1rem; background-color: #3b82f6; color: white; transition: background-color 0.3s ease; font-weight: bold; border: none; border-radius: 10px; font-size: 1.1rem; }
        .btn-login:hover { background-color: #2563eb; }
        .error-alert { color: #c0392b; margin-bottom: 1.5rem; }
        #voiceModal { display: none; position: fixed; inset: 0; background: rgba(0, 0, 0, 0.7); backdrop-filter: blur(4px); z-index: 1050; }
        #voiceContent { background: #ffffff; border: 2px solid #3b82f6; padding: 2rem; border-radius: 16px; width: 90%; max-width: 400px; margin: 6% auto; text-align: center; box-shadow: 0 12px 28px rgba(0, 0, 0, 0.3); position: relative; }
        .microphone-icon { font-size: 2rem; color: #3b82f6; margin-bottom: 0.5rem; }
        #voiceSentence { font-weight: bold; font-size: 1.4rem; color: #1f2937; margin-bottom: 1rem; }
        #voiceInput { text-align: center; font-style: italic; color: #374151; }
        #voiceStatus { margin-top: 1rem; font-weight: 600; }
    </style>
</head>
<body>
...
if (!SpeechRecognition) { alert('Tu navegador no soporta reconocimiento de voz.'); }
    ...
    recog.addEventListener('end',()=>{ const hablado=normalizar(inputEl.value), objetivo=normalizar(fraseActual); 
        if(hablado===objetivo){ validado=true; statusEl.style.color='green'; statusEl.textContent='✅ Autenticación exitosa.'; btn.textContent='Continuar'; } 
        else if(!validado){ statusEl.style.color='red'; statusEl.textContent='❌ No coincidió. Inténtalo nuevamente.'; btn.textContent='Reintentar'; } 
    });
    recog.addEventListener('error',()=>{ statusEl.style.color='red'; statusEl.textContent='⚠️ Error al escuchar. Inténtalo otra vez.'; btn.textContent='Reintentar'; });
    recog.addEventListener('speechend',()=>recog.stop());
    btn.addEventListener('click',()=>{ 
        if(validado){ const rol=<?= json_encode($_SESSION['temp_rol'] ?? '') ?>; window.location.href=window.location.pathname+'?voice=1&confirm='+encodeURIComponent(rol); }
        else{ if(btn.textContent==='Reintentar') nuevaFrase(); btn.textContent='Escuchando...'; recog.start(); }
    });
